...
const shortcutList = [
  ...
  [
    { header: "selection" },
    { key: "Ctr + A", action: "select-all" },
    { key: "Shift + Ctrl + A", action: "deselect-all" },
...
class ShortcutsPopup extends Overlay {
  constructor(args = {}) {
    ...
    shortcutList.forEach((cate) => {
      let cateDiv = new Container({ class: "shortcut-cate" });
      
      cate.forEach((shortcut) => {
        //shortcut
        if (shortcut.header) {
      ...
      shortcutsContainer.append(cateDiv);
    });
